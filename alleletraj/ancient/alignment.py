...
class AdapterRemoval(utils.PipelineTask):
    ...
    def run(self):
        ...
        cmd = [
            'AdapterRemoval',
            '--trimns',                       # trim ambiguous bases (N) at 5'/3' termini
            '--trimqualities,'                # trim bases at 5'/3' termini with low quality scores 
            '--minquality', TRIM_MIN_BASEQ,   # the min quality score for 5'/3' termini bases
            '--minlength',  TRIM_MIN_LENGTH,  # reads shorter than this length are discarded following trimming
            '--gzip',
        ]
